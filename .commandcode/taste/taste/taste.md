...
- Wants source-code comments written in Chinese; asks for existing English comments to be translated to Chinese while keeping the code itself untouched. Confidence: 0.65
- Expects comment localization to cover a whole module, not a single file — after the .c is translated he says "继续翻译" to have the matching header's doc blocks, parameter comments and repeated return-value sections translated too, while leaving identifiers and ASCII signal/timing diagrams byte-for-byte intact. Confidence: 0.5
